...
function parseCSV(csv: string): Record<string, HistoricalDataType[]> {
    ...
    for (let i = 1; i < lines.length; i++) {
        ...
        const entry: HistoricalDataType = {
...
async function getSimulationData(columns: string[], rowConstraints: string[]): Promise<Record<string, HistoricalDataType[]>> {
    try {
        const response = await axios.post('/api/get_simulation_data',
            {columns: columns, row_constraints: rowConstraints},
